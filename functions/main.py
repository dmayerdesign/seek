...
def getTeacherData(request: https_fn.CallableRequest):
    ...
    if user is not None and user.email is not None:
        ...
        if len(teacherMatches) == 1:
            ...
            lessons = list(lessons_coll.stream())
            if len(lessons) > 0:
                ...
                for lesson in teacher.lessons:
                    ...
                    responses = list(responses_coll.where(field_path="deleted", op_string="in", value=[False, None]).stream())
                    if len(responses) > 0:
...
def _getLessons(
    ...
    if teacher is not None and teacher.id is not None:
        ...
        if lesson_id is not None:
            lessons_data = [
                lessons_coll.document(lesson_id).get().to_dict()
            ]
        else:
            lessons_data = list(lessons_coll.stream())
            ...
            if include_responses:
                for plan in teacher.lessons:
                    ...
                    if len(responses) > 0:
                        ...
                        plan['responses'] = [responses[i].to_dict() for i in range(len(responses))]
                        # Sort by created_at
                        plan['responses'].sort(key=lambda question: question.created_at)
...
# Meant to be called by the student (so needs to be public)
@https_fn.on_call(cors=options.CorsOptions(cors_origins=["*"]))
def getLesson(request: https_fn.CallableRequest):
    ...
    if teacher_email is not None:
        ...
        if len(teacherMatches) == 1:
            teacher = TeacherData(**teacherMatches[0].to_dict())
            return _getLessons(teacher, False, lesson_id)
    raise https_fn.HttpsError(code=https_fn.FunctionsErrorCode.FAILED_PRECONDITION, message="invalid request")
                        ...
                        for question in lesson_questions:
                            ...
                            for resp in responses:
                                lesson_responses_message_content.append({
                                    "type": "text",
                                    "text": f"{resp.student_name} answered: {resp.analysis.response_summary}",
                                })
                            llm_messages.append({
                                "role": "user",
                            ...
                            llm_messages.append({
                                ...
                                "content": dedent(f"""
                                    ...
                                    Respond with a JSON array, structured as follows:
                            ...
                            analyses_raw: list[dict[str, str]] = json.load(message.content[0].text)
                            ...
                            for analysis_raw in analyses_raw:
                                ...
                                responses_by_category[category].append(
                                    responses_by_student_name.get(student_name)
                                )
...
# Must be public, meant to be called by student
@https_fn.on_call(cors=options.CorsOptions(cors_origins=["*"]))
def putLessonResponse(request: https_fn.CallableRequest):
    lesson_resp_data = LessonResponse(**request.data)
...
def analyze_lesson_response(
    ...
    if lesson_resp_data.response_image_base64 is not None:
        llm_messages = []
        message_content = [
            {
                "type": "text",
                "text": f"I asked my high school students the following question: {question_data.body_text}",
            },
            {
                "type": "text",
        ...
        message_content.append({
            ...
            "source": {
                ...
                "data": lesson_resp_data.response_image_base64,
            },
        })
        message_content.append({
            "type": "text",
            "text": "Your response should be a summary of this drawing. If you see any text in it, include the exact text in your description. Otherwise, simply do your best to describe what you think it depicts, in one or two sentences.",
        })
        llm_messages.append({
            "role": "user",
