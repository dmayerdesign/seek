...
const LessonPlans: FC<LessonPlansProps> = ({ teacherData, setTeacherData, refreshTeacherData }) => {
	...
	const createLessonPlan = useCallback(async () => {
		if (user) {
		...
		<>
			<div className="content-gutters">
				<h2 style={{ display: "flex", justifyContent: "space-between" }}>
					Lesson plans
					<button onClick={() => createLessonPlan()}>+ Add new lesson plan</button>
				</h2>
			</div>
			...
			<div className="content-gutters">
				{teacherData?.lesson_plans?.map(
					(lp) =>
						lessonPlansCtrl[lp.id] && (
							<div key={lp.id}>
								<h3 style={{ display: "flex", justifyContent: "space-between" }}>
									<a href={`/for-teachers/lesson-plans/${lp.id}`}>{lessonPlansCtrl[lp.id].title}</a>
								</h3>
							</div>
						),
				)}
