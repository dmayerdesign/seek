...
const LessonQuestionResponses: FC<LessonQuestionResponsesProps> = ({ analysis, onReorderResponse }) => {
	...
	const [responsesByCatNameCtrl, setResponsesByCatNameCtrl] =
		useState<{ catName: string; responses: LessonResponse[] }[]>()
	useEffect(() => {
	...
	useEffect(() => {
		if (containerRefsByCatName.current) {
			dragula(
				...
				const oldCatName =
					Object.keys(responsesByCatName).find((catName) =>
						responsesByCatName[catName].find((r) => r?.student_name === studentName),
					) ?? ""
			...
			{responsesByCatNameCtrl?.map(({ catName, responses }) => (
				...
				>
					...
					{containerRefsByCatName.current && (
						<div
							className="response-category-responses"
							key={catName}
